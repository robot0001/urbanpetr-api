...
- **WAF** — shared `CLOUDFRONT`-scope WebACL (us-east-1) attached to the API CloudFront distribution: kill-switch block rule (priority 0, empty by default), IP reputation list, common rule set, known bad inputs, per-IP rate limit (1000 req / 5 min), a tighter per-IP rate limit (100 req / 5 min) scoped to football-api's auth routes (`/v1/session`, invite redemption), and a third scoped to football-api's anonymous read surface (`/v1/public/`, 500 req / 5 min) — **the last one counts rather than blocks**, so it is a measurement of what a real reader behind a carrier NAT spends and not yet a ceiling; the 1000/5min rule is what blocks in the meantime, and `PublicReadRateLimit` in CloudWatch is the number that decides when to turn it on
